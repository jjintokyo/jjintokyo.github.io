...
Xmas_mode=1; page=1; preset=0; stop=0; volume=10; volume_step=1;
...
t2m_Xmas_mode="radio switch to christmas mode"; t2m_regular_mode="radio switch to regular mode";
...
mouse_button_right_left='*BTN_RIGHT+BTN_LEFT	2	command*'
mouse_button_left_middle='*BTN_LEFT+BTN_MIDDLE	2	command*'
mouse_button_right_middle='*BTN_RIGHT+BTN_MIDDLE	2	command*'
mouse_button_middle='*BTN_MIDDLE	2	command*'
mouse_button_middle_left='*ABS_MISC	2	command*'
mouse_button_middle_right='*ABS_MISC	1	command*'
...
initialization()     { /bin/echo "Xmas.sh is starting..... `/bin/date`" > /dev/kmsg;
                       stop_radio;
                       while true; do /bin/ping -c1 google.com &>/dev/null && break; /usr/bin/aplay "$connect_to_wifi" &>/dev/null; /bin/sleep 1; done;
                       /usr/sbin/ntpd -n -q -p 0.openwrt.pool.ntp.org -p 1.openwrt.pool.ntp.org -p 2.openwrt.pool.ntp.org;
                       ...
                       set_volume "$volume"; if [ "$Xmas_mode" -eq 1 ]; then christmas_mode; else regular_mode; fi; }
...
christmas_mode()     { stop_radio; talk_2_me "$t2m_Xmas_mode";
                       /usr/bin/mpc clear &>/dev/null; /bin/cat /root/my_radio/christmas.db | /usr/bin/mpc add &>/dev/null; /usr/bin/mpc repeat on &>/dev/null;
...
while true; do
   ...
   /usr/sbin/thd --dump $input_devices | /usr/bin/awk "/^# BTN|^# REL_WHEEL|^# ABS|^# KEY/{print}" > $my_radio_pipe &
   while read line; do
      case $line in
         ($mouse_button_left)           if [ "$stop" -eq 0 ]; then play_next; else radio_play_stop; fi;;
         ($mouse_button_right)          if [ "$stop" -eq 0 ]; then play_previous; else radio_play_stop; fi;;
         ($mouse_button_left_right)     if [ "$stop" -eq 1 ]; then power_down_system; fi; break;;
         ($mouse_button_right_left)     if [ "$stop" -eq 1 ]; then power_down_system; fi; break;;
         ($mouse_button_left_middle)    christmas_mode; break;;
         ($mouse_button_right_middle)   regular_mode; break;;
         ($mouse_button_middle)         radio_play_stop; break;;
         ($mouse_button_middle_left)    if [ "$stop" -eq 1 ]; then play_mp3; fi;;
         ($mouse_button_middle_right)   if [ "$stop" -eq 1 ]; then random_everything; fi;;
         ...
         ($keypad_special_13)           christmas_mode; break;;
