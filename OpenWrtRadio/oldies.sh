...
#                                                  #
#--------------------------------------------------#
#  Handles the Wired Mouse + the Wireless Keypad   #
#--------------------------------------------------#
#                                                  #
#              JJR / Mon Jan 10 2022               #
#                                                  #
...
oldies_mode=0; page=1; preset=0; stop=0; volume=10; volume_step=1;
oldies_pipe="/tmp/oldies_pipe"; input_devices="/dev/input/event*";
...
mouse_button_left_right='*BTN_LEFT+BTN_RIGHT	1	command*'
mouse_button_left_middle='*BTN_LEFT+BTN_MIDDLE	1	command*'
mouse_button_left_top='*ABS_MISC	129	command*'
mouse_button_right='*BTN_RIGHT	0	command*'
mouse_button_right_left='*BTN_RIGHT+BTN_LEFT	1	command*'
mouse_button_right_middle='*BTN_RIGHT+BTN_MIDDLE	1	command*'
mouse_button_right_top='*ABS_MISC	130	command*'
mouse_button_middle='*BTN_MIDDLE	0	command*'
mouse_button_middle_left='*BTN_MIDDLE+BTN_LEFT	1	command*'
mouse_button_middle_right='*BTN_MIDDLE+BTN_RIGHT	1	command*'
mouse_button_middle_top='*ABS_MISC	132	command*'
mouse_button_top='*ABS_MISC	128	command*'
mouse_button_side_1='*BTN_EXTRA	0	command*'
mouse_button_side_1_left='*BTN_EXTRA+BTN_LEFT	1	command*'
mouse_button_side_1_right='*BTN_EXTRA+BTN_RIGHT	1	command*'
mouse_button_side_1_middle='*BTN_EXTRA+BTN_MIDDLE	1	command*'
mouse_button_side_1_top='*ABS_MISC	144	command*'
mouse_button_side_1_side_2='*BTN_EXTRA+BTN_SIDE	1	command*'
mouse_button_side_2='*BTN_SIDE	0	command*'
mouse_button_side_2_left='*BTN_SIDE+BTN_LEFT	1	command*'
mouse_button_side_2_right='*BTN_SIDE+BTN_RIGHT	1	command*'
mouse_button_side_2_middle='*BTN_SIDE+BTN_MIDDLE	1	command*'
mouse_button_side_2_top='*ABS_MISC	136	command*'
mouse_button_side_2_side_1='*BTN_SIDE+BTN_EXTRA	1	command*'
...
keypad_special_12='*KEY_NUMLOCK+KEY_KPSLASH	1	command*'
keypad_special_13='*KEY_NUMLOCK+KEY_KPASTERISK	1	command*'
keypad_special_14='*KEY_NUMLOCK+KEY_KPENTER	1	command*'

initialization()     { /bin/echo "oldies.sh is starting..... `/bin/date`" > /dev/kmsg;
                       stop_radio;
...
while true; do
   /usr/bin/killall -q thd; /bin/rm -f $oldies_pipe; /usr/bin/mkfifo $oldies_pipe;
   /usr/sbin/thd --dump $input_devices | /usr/bin/awk "/^# BTN|^# REL_WHEEL|^# ABS|^# KEY/{print}" > $oldies_pipe &
   while read line; do
      case $line in
         ($mouse_button_left)           play_preset "$preset";;
         ($mouse_button_right)          radio_play_stop;;
         ($mouse_button_middle)         random_everything;;
         ($mouse_button_top)            random_by_genre "5";;
         ($mouse_button_side_1)         page_up;;
         ($mouse_button_side_2)         preset_up;;
         ($mouse_button_side_1_left   | $mouse_button_side_1_right | $mouse_button_side_1_middle) page_down; /bin/sleep 1; break;;
         ($mouse_button_side_2_left   | $mouse_button_side_2_right | $mouse_button_side_2_middle) preset_down; /bin/sleep 1; break;;
         ($mouse_button_side_1_side_2 | $mouse_button_side_2_side_1)                              power_down_system; break;;
         ($mouse_button_left_middle   | $mouse_button_left_right)      if [ "$stop" -eq 0 ]; then play_next; else /bin/sleep 1; fi; break;;
         ($mouse_button_right_middle  | $mouse_button_right_left)      if [ "$stop" -eq 0 ]; then play_previous; else /bin/sleep 1; fi; break;;
         ($mouse_button_middle_left   | $mouse_button_middle_right)    if [ "$stop" -eq 1 ]; then say_time; else /bin/sleep 1; fi; break;;
         ($mouse_button_left_top)       regular_mode; break;;
         ($mouse_button_right_top)      oldies_mode; break;;
         ($mouse_button_middle_top)     play_mp3; break;;
         ($mouse_button_side_1_top)     stop_radio; eval "$speaker_test"; stop=0; /bin/sleep 1; break;;
         ($mouse_button_side_2_top)     update_database; break;;
         ($mouse_wheel_up)              volume_down "$(($volume_step*3))";;
         ($mouse_wheel_down)            volume_up "$(($volume_step*3))";;
         ...
         ($keypad_slash)                play_previous;;
         ($keypad_asterisk)             play_next;;
         ...
         ($keypad_special_12)           regular_mode; break;;
         ($keypad_special_13)           oldies_mode; break;;
         ($keypad_special_14)           stop_radio; eval "$speaker_test"; break;;
      esac
   done < $oldies_pipe
done
