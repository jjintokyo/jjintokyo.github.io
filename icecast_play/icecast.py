...
if (len(sys.argv) < 2):
    # genre = 'classical'
    source = 'ice'
    c.execute('''select * from ( select listen_url, source from channels where source = ?) order by RANDOM() limit 1''',[(source.lower())])
else:
    genre = sys.argv[1]
...
url = c.fetchone()[0]
print(url)
cmd = 'vlc -I dummy -q --alsa-audio-device equal ' + url + ' </dev/null &>/dev/null &'
os.system(cmd)
sys.exit(0)
